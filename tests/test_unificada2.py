...
def main():
    repo_alumno_url = "https://github.com/fake-org/fake-repo2.git"
    ...
    print("--- GIT LOG ---")
    print(run(f"git -C {destino_local} log --oneline -2"))
    print("--- LINE COUNT ---")
    fecha = datetime.datetime.utcnow().strftime("%Y-%m-%d")
    print(run(f"git -C {destino_local} show HEAD:logs/interacciones/{fecha}.jsonl | wc -l"))
    print("--- JSONL CONTENT ---")
    print(run(f"cat {destino_local}/logs/interacciones/{fecha}.jsonl"))
